<template>
	<view class="easy-select" @click.stop="trigger" :style="[easySelectSize]">
		<input type="text" v-model="value" :placeholder="placeholder" disabled clearable :class="size=='ziji'?'text-one':''">
		...
		<view class="easy-select-options" v-if="showOptions" :style="{'min-width': boundingClientRect.width + 'px', top: optionsGroupTop, margin: optionsGroupMargin}">
			<view class="easy-select-options-item" v-for="item in options" :key="item.id" @click.stop="select(item)" :class="{active: currentSelect.id === item.id}">
				<!-- <text>{{item.realName}}</text> -->
				<view class="flex align-center"  v-if="size=='ziji'">
					{{item.name}}
				</view>
				<view class="flex align-center" v-if="size!='ziji'">
					<image v-if="item.headimgUrl" class="margin-right-xl" :src="item.headimgUrl" style="width: 50rpx; height: 50rpx;"></image>
